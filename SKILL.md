...
将月度项目差旅费的电子发票 PDF、行程单、付款截图、发票扫描件整合为可直接提交财务的 Excel。**Tesseract OCR 发票号精确匹配**（经验覆盖率 ~70%）为扫描件主路径，Read 工具 / AI 视觉为辅助。可批量处理 200+ 张。
...
| 滴滴 | `滴滴电子发票 (N).pdf` + `滴滴出行行程报销单 (N).pdf`，不静默自动分配，生成 `pdf_mapping_review.json` 待人工确认后写入 `pdf_mapping.json` |
| 高德聚合 | `【{平台}-{金额}】高德打车电子发票.pdf` + 行程单，平台名匹配 |

**滴滴 PDF 人工确认流程：**

当滴滴 PDF 或其他无法通过明确规则自动匹配的 PDF 出现时，`build.py` 会生成 `pdf_mapping_review.json`。用户需要根据 review 清单人工确认归属，并写入 `pdf_mapping.json`。再次运行 `build.py` 后，已人工确认的 PDF 不应再被判定为孤儿 PDF。

`pdf_mapping.json` 格式：

```json
{
  "6": ["滴滴电子发票.pdf", "滴滴出行行程报销单.pdf"],
  "10": ["滴滴电子发票 (2).pdf", "滴滴出行行程报销单 (2).pdf"]
}
```

- key 是 Excel 行号，例如 R6 写成 `"6"`
- value 必须是 PDF 文件名数组（数组，不是字符串）
- 同一个 PDF 不允许映射到多个行
- 引用的所有 PDF 文件名必须存在于工作目录中

### 阶段 4：付款截图

**方案 A — Read 工具辅助识别（可用时优先）：**
...
| Phase 3 消元 | 剩余按行需求分配：先补发票位，再补行程单位 | ~100%（消元分配不保证准确性，需人工抽查） |
